...
open class ENSideMenu : NSObject, UIGestureRecognizerDelegate {
    ...
    open var menuHeight : CGFloat = 300.0 {
        didSet {
            needUpdateApperance = true
            updateSideMenuApperanceIfNeeded()
            updateFrame()
    ...
    public init(sourceView: UIView, menuPosition: ENSideMenuPosition, blurStyle: UIBlurEffectStyle = .light) {
        ...
        // Add top swipe gesture recognizer
        let downSwipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action: #selector(ENSideMenu.handleGesture(_:)))
        leftSwipeGestureRecognizer.delegate = self
        leftSwipeGestureRecognizer.direction = UISwipeGestureRecognizerDirection.down
        ...
        case .top:
            sideMenuContainerView.addGestureRecognizer(downSwipeGestureRecognizer)
    ...
    func updateFrame() {
        ...
        case .right:
            menuFrame = CGRect(
                x: isMenuOpen ? width - menuWidth : width+1.0,
                y: sourceView.frame.origin.y,
                width: menuWidth,
                height: height
            )
    ...
    fileprivate func toggleMenu (_ shouldOpen: Bool) {
        ...
        if (bouncingEnabled) {
            ...
            var gravityDirectionX = CGFloat(0)
            var gravityDirectionY = CGFloat(0)
            ...
            case .right:
                ...
                boundaryPointY =  -25
            case .top:
                gravityDirectionY = (shouldOpen) ? -1 : 1
                pushMagnitude = (shouldOpen) ? -35 : 35
                boundaryPointX =  25
                boundaryPointY = (shouldOpen) ? menuHeight : -height-menuHeight-2
            }
            
            let gravityBehavior = UIGravityBehavior(items: [sideMenuContainerView])
            gravityBehavior.gravityDirection = CGVector(dx: gravityDirectionX,  dy: gravityDirectionY)
    ...
    open func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if gestureRecognizer is UISwipeGestureRecognizer {
            ...
            case .top:
                if isMenuOpen {
                    if touchPosition.y > menuHeight * 4/5 {
                        return true
                    }
                }
                else {
    ...
    @objc internal func handleGesture(_ gesture: UISwipeGestureRecognizer) {
        toggleMenu((self.menuPosition == .right && gesture.direction == .left)
            ...
            || (self.menuPosition == .top && gesture.direction == .down))
    }
    
    @objc internal func handlePan(_ recognizer : UIPanGestureRecognizer){
        
        let panDirectionVector = recognizer.velocity(in: recognizer.view)
        ...
        case .changed:
            ...
            let translation = recognizer.translation(in: sourceView)
            var point = sideMenuContainerView.center + translation
            switch(menuPosition) {
            case .left:
                point += CGPoint(x: menuWidth / 2, y: 0)
            case .right:
                point += CGPoint(x: -menuWidth / 2, y: 0)
            case .top:
                point += CGPoint(x: 0, y: menuHeight / 2)
            }
            
            switch(menuPosition) {
            case .left:
                if point.x <= 0 || point.x > self.sideMenuContainerView.frame.width {
                    return
                }
                sideMenuContainerView.center += CGPoint(x: translation.x, y: 0)
            case .right:
                if point.x <= sourceView.frame.size.width - menuWidth || point.x >= sourceView.frame.size.width
                {
                    return
                }
                sideMenuContainerView.center += CGPoint(x: translation.x, y: 0)
            case .top:
                if point.y <= 0 || point.y > self.sideMenuContainerView.frame.height {
                    return
                }
                sideMenuContainerView.center += CGPoint(x: 0, y: translation.y)
            }
        ...
        default:
            ...
            let leftToRight = panDirectionVector.x > 0
            let upToDown = panDirectionVector.y > 0
            
            switch(menuPosition) {
            case .left:
                shouldClose = !leftToRight && sideMenuContainerView.frame.maxX < menuWidth
            case .right:
                shouldClose = leftToRight && sideMenuContainerView.frame.minX >  (sourceView.frame.size.width - menuWidth)
            case .top:
                shouldClose = !upToDown && sideMenuContainerView.frame.maxY < menuHeight
            }
